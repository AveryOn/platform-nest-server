...
import { ConfigPaginatorInput, ConfigPaginatorData, PaginationMeta, ResponsePaginatorInput } from '~/shared/paginator/application/paginator.types'
...
export class PaginatorService implements PaginatorServicePort {
  ...
  public config({ limit, page }: ConfigPaginatorInput): ConfigPaginatorData {
    ...
    if(!numCheck(this.page).success || !numCheck(this.limit).success) {
      throw new AppError(ERROR.INVALID_DATA, this.logger).log('', { scope: 'paginator.BuildConfig', context: { page: this.page, limit: this.limit } })
    }
    ...
    if(!numCheck(skip, 0).success || !numCheck(take).success) {
      throw new AppError(ERROR.INVALID_DATA, this.logger).log('', { scope: 'paginator.BuildConfig', context: { skip, take } })
    }
  ...
  public response<T>({ data, total }: ResponsePaginatorInput<T>): PaginationMeta {
    ...
    if(!numCheck(totalPages, 0).success) {
      throw new AppError(ERROR.INVALID_DATA, this.logger).log('', { scope: 'paginator.Response', context: { totalPages } })
    }
