...
import { createdAt, id, referenceOnUUID, updatedAt } from '~/infra/drizzle/drizzle.helpers'
import { projectsTable, ruleGroupsTable } from '~/infra/drizzle/schemas'

export const projectRuleGroupConfigStatus = pgEnum('project_rule_group_config_status', [
  'active',
  'hidden',
])
...
export const projectRuleGroupConfigsTable = pgTable(
  'project_rule_group_configs',
  {
    ...
    status: projectRuleGroupConfigStatus('status').default('active').notNull(),
    createdAt: createdAt(),
    updatedAt: updatedAt(),
  },
  (t) => [
    uniqueIndex('prgc_project_rule_group_unique').on(t.projectId, t.ruleGroupId),
